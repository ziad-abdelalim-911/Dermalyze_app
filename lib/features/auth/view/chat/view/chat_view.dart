          ...
          body: Container(
            decoration: BoxDecoration(
              image: DecorationImage(
                image: const AssetImage('assets/img/chat_bg.png'),
                    ...
                    if (_showEmoji)
                      SizedBox(
                        ...
                        child: EmojiPicker(
                          ...
                          config: const Config(
                              height: 250, checkPlatformCompatibility: true),
                        ),
                      ),
                  ],
                ),
                ...
                children: ['👍', '❤️', '😂', '😮', '😢', '🙏'].map((emoji) {
                  return GestureDetector(
                    onTap: () {
                      ...
                      if (message.id != null) {
                        context.read<ChatCubit>().reactToMessage(message.id!, emoji);
                      }
                    },
                    child: Container(
...
class BubblePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final path = Path();
    if (isMe) {
      // Sent message (right side): tail points right
      // Flat side on the left (touching the bubble)
      path
        ..moveTo(0, 0)
        ..lineTo(size.width, 0) // tip pointing right
        ..lineTo(0, size.height); // straight down on left side
    } else {
      // Received message (left side): tail points left
      // Flat side on the right (touching the bubble)
      path
        ..moveTo(size.width, 0)
        ..lineTo(0, 0) // tip pointing left
        ..lineTo(size.width, size.height); // straight down on right side
    }
    path.close();
